...
class AgenciaNaviera(models.Model):
    _name = 'sigmap.agencia.naviera'
    _description = _('Shipping Agency')
    ...
    partner_id = fields.Many2one(
        ...
        help=_('Information of the partner related to the Shipping Agency'))
    ...
    tipo_agencia = fields.Selection(
        [
            ('nav', 'NAV'),
            ('est', 'EST'),
            ('mue', 'MUE'),
        ], string=_('Shipping Agency Type'),
        default='nav',
        required=True,
        index=True,
        copy=False,
    ...
    representante_legal = fields.Many2one(
        ...
        help=_('The legal representative of the Shipping Agency'))
    """
    ...
    matricula_agencia_ids = fields.One2many(
        string=_('Registration numbers'),
        comodel_name='sigmap.matricula.agencia',
    ...
    def _onchange_partner_id(self):
        ...
        if self.partner_id:
            ...
            agencies_for_partner = self.env['sigmap.agencia.naviera'].search([('partner_id.vat', '=', partner_vat)])
            if len(agencies_for_partner) == 0:
                computed_value = True
            if len(agencies_for_partner) == 1:
                agency_id = agencies_for_partner[0].id
                if type(self.id) == int:
                    computed_value =  self.id == agency_id
                else:
                    computed_value = self.id.origin == agency_id

        self.partner_is_valid = computed_value

    @api.depends('matricula_agencia_ids')
    def _compute_matricula(self):
        for rec in self:
            ...
            if rec.matricula_agencia_ids:
                current_matricula = [ reg_num for reg_num in rec.matricula_agencia_ids if reg_num['active']]
                ...
                if len(current_matricula) < len(rec.matricula_agencia_ids):
