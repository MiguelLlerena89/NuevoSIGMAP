...
class SRIPosDocumento(models.Model):
    _name = "l10n_ec.sri.punto.emision.documento"
    _description = 'Autorización'
    _rec_name = 'numero'

    def _default_partner(self):
        if self._context.get('pos_doc_tipo') == 'out':
            partner = self.env.user.company_id.partner_id
            return partner
    ...
    pos_doc_tipo = fields.Selection([
            ('in',''),
            ('out','Out'),
            ], index=True, change_default=True,
        default=lambda self: self._context.get('pos_doc_tipo', 'out'),
        )
    ...
    sequence = fields.Integer('Secuencia', related='sequence_id.number_next_actual', readonly=False, size=9, copy=False)
    ...
    _sql_constraints = [
        ...
        ('numero_unique', 'unique(partner_id,numero, pos_doc_tipo, l10n_latam_document_type_id, punto_emision, establecimiento)', 'Autorización es única para la misma empresa!'),
        ]
    ...
    def create(self, vals):
        ...
        partner = self.env['res.partner'].browse(partner_id)
        self.check_vat(partner.vat)
        auth = super().create(vals)

        if vals.get('pos_doc_tipo') == 'out':
    ...
    def write(self, vals):
        if self.partner_id:
            ...
            partner = self.env['res.partner'].browse(partner_id)
        self.check_vat(partner.vat)
        auth = super().write(vals)
        if self.pos_doc_tipo == 'out':
    ...
    def _create_sequence(self, vals):
        ...
        pos_doc_tipo = self.env['l10n_latam.document.type'].search([('id', '=', vals.get('l10n_latam_document_type_id', self.l10n_latam_document_type_id.id))], limit=1)
        ...
        if 'numero' not in vals:
            if not vals['numero']:
                vals['numero'] = vals['establecimiento'] + vals['punto_emision'] +  pos_doc_tipo.code
        else:
            vals['numero'] = ''

        seq = {
            'name': pos_doc_tipo.name.upper() + is_elect + vals['numero'],
